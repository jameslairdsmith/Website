...
author: "EE"
date: "2020-05-06"
description: "Examples & tutorial writing rolling aggregate functions"
...
The first step, then, is just to test the logic of the loop I need to create to calculate rolling averages. I'll do this by assuming a 28 day window (we'll be able to change the window later), create a "truth" to test against using RcppRoll's `roll_mean()` function, and then writing my loop using `map()`.
```{r test loop}
...
Great, so the logic of the loop works. Now, let's extend it a little bit to create a function where I can specify the variable I want to use as well as the window I want to take the rolling average over.
